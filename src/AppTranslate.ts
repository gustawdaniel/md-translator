...
export class AppTranslate {
    ...
    async translateSentence(
        ...
        ).get({ sourceLang, targetLang, sourceText }) as
            | TranslationDbRow
            | undefined;
        ...
        if (!dbResponse) {
            throw new Error(
                'Failed to insert the translation into the database.',
            );
        }

        console.log(
            `Translation: ${sourceLang} => ${targetLang} saved [${
                sourceText.substring(0, 20).replaceAll('\n', '')
            } => ${
                targetText.substring(0, 20).replaceAll('\n', '')
            }]`,
        );
